...
#Remove extraneous columns from the med tabs by selecting specific tabs
...
pt_fractures <- (x %>% 
    ...
    filter((grepl('M80', CODE))) %>% 
    filter((grepl('fracture', DESC))) %>% 
    filter((grepl('Admission', TEXT_VALUE))) %>%
    arrange(DATE))
...
time_from_fracture <- as.double.difftime(
  (
    (
      Fracture_Dates %>% filter(ID == id) %>% select(DATE)
      )[[1]]
    ) - DM_Dx_date
  )/365.25
...
Fracture_Dates <- px %>% left_join(FRAX, by= c("GENDER","RACE","AGE","CRF","BMI"), copy = FALSE, keep = FALSE)%>%
