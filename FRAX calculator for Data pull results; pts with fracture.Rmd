...
percent_missing_FRAX <- (nrow(fx)/nrow(Fracture_Dates_fx))*100
...
Fracture_dates_final <- transform((Fracture_Dates_fx), ID = as.character(ID))
...
for (i in 1:nrow(Fracture_Dates)){
  id <- as.double(Fracture_Dates$ID[i])
...
```{r CCI}
#Import the 'Fracture_Dates' and 'CCI_acriscit'
Fracture_Dates <- Fracture_Dates[-1]
...
Fracture_Dates_CCI <- Fracture_Dates %>%
