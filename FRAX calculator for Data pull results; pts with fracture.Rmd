...
Glucocorticoid_start_date <- (
  Glucocorticoid_data %>%
    arrange(DATE) %>%
    select(DATE)
  )[[1,1]]
Glucocorticoid_stop_date <- (
  Glucocorticoid_data %>%
    arrange(desc(DATE)) %>%
    select(DATE)
  )[[1,1]]
Glucocorticoid_duration <- (
  Glucocorticoid_stop_date - Glucocorticoid_start_date
  )
test <- as.double(Glucocorticoid_data)[1]
Glucocorticoid_duration <- ifelse(
  test = "NA", 
  "NA", Glucocorticoid_duration
  )
...
osteo_med <- ifelse(osteo_med[1,1] == 'NA', 'NA', osteo_med) %>% as.tibble()

osteo_med_start_date <- ifelse(
  osteo_med[1,1] == 'NA','NA',(osteo_med %>% 
                           select(DATE))[[1,1]]
                           )
osteo_med_start_dose <- ifelse(
  osteo_med[1,1] == 'NA','NA',(osteo_med %>% 
                           select(NUMERIC_VALUE))[[1,1]]
)
osteo_med_last_date <- ifelse(
  osteo_med[1,1] == 'NA','NA',(osteo_med %>% 
                           arrange(desc(DATE)) %>% 
                           select(DATE))[[1,1]]
  )
