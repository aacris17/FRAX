...
thirteen_patients <- filename_patients
...
rm(filename, filename_patients, filename_visits, test2)
...
names(thirteen_patients) <- col_names[1:ncol(thirteen_patients)]
  thirteen_patients <- thirteen_patients %>%
    mutate(BIRTH_DATE = mdy_hm(BIRTH_DATE) %>% as.Date()) %>%
    mutate(DEATH_DATE = mdy_hm(DEATH_DATE) %>% as.Date())
  
tibble_names <- thirteen
Patients <- thirteen_patients
rm(thirteen, thirteen_patients)
```
