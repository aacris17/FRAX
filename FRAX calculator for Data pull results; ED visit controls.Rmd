...
thirteen <- ("H:/Health Control (Hip fx)/2013/2013_acriscit.xlsx")
...
#thirteen_visits <- filename_visits

Patients <- thirteen$Patients
Visits <- thirteen$Visits
thirteen <- within(thirteen, rm(Patients, Visits))
...
for (i in 1:length(tibble_titles)) {
 ...
 x <- thirteen[tab_names_x$value] %>% bind_rows()
...
#Rename all the columns in thirteen and change DATE to a date format
col_names <- c(
  "ID", 
  "VISIT", 
  "DATE",
...
for (i in 1:length(thirteen)){
  names(thirteen[[i]]) <- col_names[1:ncol(thirteen[[i]])]
  thirteen[[i]]$DATE <- thirteen[[i]]$DATE %>% mdy_hm(quiet = TRUE) %>% as.Date()
}
...
```
Remove patients that do not meet visit criteria: 3 prior visits, one follow-up
```
...
Visits
```{r Visits}
#Ensure that the column names listed below match the column names in your actual data (ie the tibble 'Visits')
col_names <- c(
  "ID", 
  "VISIT", 
  "START_DATE",
