...
Exp <- chi$observed[2]/nrow(test2 %>% filter(DM == "FALSE"))*100
Control <- chi$observed[4]/nrow(test2 %>% filter(DM == "TRUE"))*100
p.value <- chi$p.value
...
geom_boxplot() +
  ...
  scale_x_discrete(labels=c("F Fracture","M Fracture","F Control","M Control"))
...
ggplot(data = smooth, aes(DM_Control,ageatindex)) +
geom_boxplot() +
  labs(title = "Diabetic Status vs Age at Index Date", x="", y="Age") +
  scale_x_discrete(labels=c("Fracture nonDiabetic","Fracture Diabetic","Control nonDiabetic","Control Diabetic"), guide = guide_axis(n.dodge=2))

#PLOT6: Sex and DM as a %
plot_tibble <- plot_tibble %>% add_row(
  Descriptor = "PLOT6")

Prop <- smooth %>% count(Fracture, DM_SEX, DM_Control, DM, GENDER)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = DM_Control, y = Percent, fill = GENDER)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Sex and Diabetes", x="", y="Percent (%)", fill= "Sex") +
  scale_x_discrete(labels=c("Healthy Control","Diabetic Control","Healthy Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))
...
Prop <- smooth %>% filter(Og_RACE == "W" | Og_RACE == "B") %>% count(Fracture, DM_SEX, DM_Control, DM, Og_RACE)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = DM_Control, y = Percent, fill = Og_RACE)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Race and Diabetes", x="", y="Percent (%)", fill= "RACE") +
  scale_x_discrete(labels=c("Healthy Control","Diabetic Control","Healthy Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))
...
Prop <- smooth %>% count(Fracture, DM)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = DM, y = Percent, fill = Fracture)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Diabetes", x="", y="Percent (%)", fill= "Fracture") +
  scale_x_discrete(labels=c("Healthy","Diabetic"))
...
#PLOT12: Age by decade vs Frax by diabetes
smooth <- smooth %>% mutate(Decade = floor(ageatindex/10)*10)

ggplot(data = smooth) +
  geom_bar(aes(x = Decade, y = Risk, fill = DM_Control), position="dodge", stat="identity") +
  labs(title = "Age vs FRAX", x="Age by Decade", y="FRAX", fill= "RACE") +
  geom_errorbar(aes(x = Decade, ymin = Range_lower, ymax = Range_upper))
