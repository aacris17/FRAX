---
title: "Code for descriptive characteristics"
author: "Anthony Criscitiello"
date: "3/14/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
library(readxl)
library(readr)
library(lubridate)
library(tibble)
library(purrr)
library(stringr)
```
```{r 0}
DM_Dates_fx <- read_csv("C:/Users/acriscit/Documents/Results/DM_Dates_fx.csv")
Fracture_Dates_fx <- read_csv("C:/Users/acriscit/Documents/Results/Fracture_Dates_fx.csv")
Visit_Dates_fx <- read_csv("C:/Users/acriscit/Documents/Results/Visit_Dates_fx.csv")

#Load control data
DM_Dates_ed <- read_csv("C:/Users/acriscit/Documents/Results/DM_Dates_ed.csv")
EM_Dates_ed <- read_csv("C:/Users/acriscit/Documents/Results/EM_Dates_ed.csv")
Visit_Dates_ed <- read_csv("C:/Users/acriscit/Documents/Results/Visit_Dates_ed.csv")

#Create a new column in each tibble that specifies whether the individual is a control or fracture patient
DM_Dates_fx <- DM_Dates_fx %>% mutate(Control = "FALSE")
Fracture_Dates_fx <- Fracture_Dates_fx %>% mutate(Control = "FALSE")
Visit_Dates_fx <- Visit_Dates_fx %>% mutate(Control = "FALSE")
DM_Dates_ed <- DM_Dates_ed %>% mutate(Control = "TRUE")
EM_Dates_ed <- EM_Dates_ed %>% mutate(Control = "TRUE")
Visit_Dates_ed <- Visit_Dates_ed %>% mutate(Control = "TRUE")

#Rename some columns
Visit_Dates_fx <- Visit_Dates_fx %>% rename(num_visits_before = num_visits_before_frac, num_visits_after = num_visits_after_frac)
Visit_Dates_ed <- Visit_Dates_ed %>% rename(num_visits_before = num_visits_before_ed, num_visits_after = num_visits_after_ed)

#Rename columns in index tibbles
Fracture_Dates_fx <- Fracture_Dates_fx %>% rename(ageatindex = ageatfracture)
EM_Dates_ed <- EM_Dates_ed %>% rename(ageatindex = ageatEM)
DM_Dates_ed <- DM_Dates_ed %>% rename(ageatindex = ageatEM)
DM_Dates_fx <- DM_Dates_fx %>% rename(ageatindex = ageatfracture)

#Bind the corresponding control and experimental data
Fracture_Dates_fx <- bind_rows(Fracture_Dates_fx, EM_Dates_ed)
DM_Dates_fx <- bind_rows(DM_Dates_fx, DM_Dates_ed)
Visit_Dates <- bind_rows(Visit_Dates_fx, Visit_Dates_ed)

#Need a line to remove duplicate IDs, be sure to select the fracture ID

rm(EM_Dates_ed, DM_Dates_ed, Visit_Dates_ed, Visit_Dates_fx)

#be sure to load Fracture_Dates_fx, Visit_Dates and DM_Dates_fx
#remove the first column
DM_Dates_fx <- DM_Dates_fx[-1]
Fracture_Dates_fx <- Fracture_Dates_fx[-1]
Visit_Dates <- Visit_Dates[-1]

#Make a column 'DM' that indicates whether the patients have DM
DM_Dates_fx <- DM_Dates_fx %>% mutate(DM = "TRUE")
#Add that column to the index tibble 'Fracture_Dates_fx'
test <- DM_Dates_fx %>% select(ID, DM)
Fracture_Dates_fx <- Fracture_Dates_fx %>% left_join(test, by = 'ID')
Fracture_Dates_fx$DM <- ifelse(is.na(Fracture_Dates_fx$DM), "FALSE", "TRUE")
rm(test)
```
#Remove patients who fractured prior to 2013 and re-filter for age
```{r 1}
#date <- ymd_hms("2013-01-01 UTC 00:00:00")
#Fracture_Dates_fx <- Fracture_Dates_fx %>% filter(DATE > date)
#numberafter2013 <-  nrow(Fracture_Dates_fx) - nrow(Fracture_Dates_fx)
#percentafter2013 <- (nrow(Fracture_Dates_fx )/nrow(Fracture_Dates_fx))*100

x<- nrow(Fracture_Dates_fx)
Fracture_Dates_fx <- Fracture_Dates_fx %>% filter(ageatindex >= 50)
num_rem <- x - nrow(Fracture_Dates_fx)
```
#Remove all patients with less than 3 prior visits and all patients without at least 1 follow-up
#Note that this is being applied to the tibble created above
```{r 2}
test <- Fracture_Dates_fx
x <- nrow(Fracture_Dates_fx)

Fracture_Dates_fx <- Visit_Dates %>%
  filter(num_visits_before >= 3 & num_visits_after >= 1) %>%
  select(ID) %>%
  inner_join(Fracture_Dates_fx, by = 'ID', keep = FALSE)
```
#Remove control patients who actually had a pathologic fracture
```{r 2.5}
#Don't use until updated, currently too broad
Controls_with_MOF <- Fracture_Dates_fx %>% filter(!is.na(fx_DATE))
Fracture_Dates_fx <- Fracture_Dates_fx %>% filter(!grepl('[Pp]athologic fracture', fx_DESC))
```
Right now Fracture_Dates_sum contains ALL the patients in our cohort.
We should not lose any patients after this point.
It is important to note that DM_Dates_fx contain all the patients with DM, but these patients are also contained in Fracture_Dates_sum. Therefore, the sum of Fracture_Dates_fx and DM_Dates_fx should equal Fracture_Dates_sum after the next chunk.
```{r 3}
#Fracture_Dates_sum contains all patients with fractures
Fracture_Dates_sum <- Fracture_Dates_fx

Health_Control <- Fracture_Dates_sum %>% filter(Control == "TRUE" & DM == "FALSE") %>% nrow() #of controls without DM
DM_Control <- Fracture_Dates_sum %>% filter(Control == "TRUE" & DM == "TRUE") %>% nrow() #of controls with DM
Health_fx <- Fracture_Dates_sum %>% filter(Control == "FALSE" & DM == "FALSE") %>% nrow() #of Exp without DM
DM_fx <- Fracture_Dates_sum %>% filter(Control == "FALSE" & DM == "TRUE") %>% nrow() #of Exp with DM
```
Create a tibble that does not include patients with DM
```{r 3.5}

#Apply the prior criteria to DM_Dates_fx
#DM_Dates_fx will hold this data
test <- Fracture_Dates_fx %>% filter(DM == "TRUE") %>% select(ID)
DM_Dates_fx <- DM_Dates_fx %>% inner_join(test, by = 'ID')
rm(test)
#Remove patients with DM from Fracture_Dates_fx
Fracture_Dates_fx <- Fracture_Dates_fx %>% filter(DM == "FALSE")

patients_left <- nrow(DM_Dates_fx) + nrow(Fracture_Dates_fx)
#Find out why this number is different from nrow(Fracture_Dates_sum), we should not have lost any patients in this process.
```
#Output some basic descriptive stats
```{r 4}
#Change Control == ... to TRUE or FALSE depending on which cohort you want to see
Desc <- Fracture_Dates_fx %>% filter(Control == "TRUE") %>% summary()
names(dimnames(Desc)) <- c("x", "y")
Desc <- as_tibble(Desc)
Desc <- Desc[-1]
Desc <- separate(
  Desc, 
  "n",
  c("m", "n"),
  sep = ":",
  remove = TRUE,
  convert = TRUE) %>%
  na.omit() %>%
  pivot_wider(
    names_from = m, 
    values_from = n
              )

write.csv(Desc, "C:\\Users\\acriscit\\Documents\\Control Desc.csv")
```
Descriptive stats for DM, same code as above
```{r 5}
#Change Control == ... to TRUE or FALSE depending on which cohort you want to see
Desc <- DM_Dates_fx %>% filter(Control == "FALSE") %>% summary()
names(dimnames(Desc)) <- c("x", "y")
Desc <- as_tibble(Desc)
Desc <- Desc[-1]
Desc <- separate(
  Desc, 
  "n",
  c("m", "n"),
  sep = ":",
  remove = TRUE,
  convert = TRUE) %>%
  na.omit() %>%
  pivot_wider(
    names_from = m, 
    values_from = n
              )

write.csv(Desc, "C:\\Users\\acriscit\\Documents\\DMExp Desc.csv")

rm(Desc)
```
ID missing data
#Change true, false or non to select the population who is missing data
#Change the tibble to determine what data you want to work with
```{r missing_data}
test <- Fracture_Dates_sum %>% filter(Control == "TRUE")

tot_na <- (test %>% filter(is.na(Risk)) %>% nrow())/nrow(test)
CODE_na <- (test %>% filter(is.na(CODE)) %>% nrow())/nrow(test)
HT_na <- (test %>% filter(is.na(HT.in)) %>% nrow())/nrow(test)
WT_na <- (test %>% filter(is.na(WT.in)) %>% nrow())/nrow(test)
AGE_na <- (test %>% filter(is.na(AGE)) %>% nrow())/nrow(test)
GENDER_na <- (test %>% filter(is.na(GENDER)) %>% nrow())/nrow(test)
Og_RACE_na <- (test %>% filter(is.na(Og_RACE)) %>% nrow())/nrow(test)

missing_data <- tibble(
  total = as.double(tot_na),
  Diagnosis = as.double(CODE_na),
  Height = as.double(HT_na),
  Weight = as.double(WT_na),
  Age = as.double(AGE_na),
  Sex = as.double(GENDER_na),
  Race = as.double(Og_RACE_na)
)
write.csv(missing_data, "C:\\Users\\acriscit\\Documents\\Control missing_data.csv")
rm(missing_data)
rm(test)

```
t.tests and chi sq tests
example:
G_v_Risk <- tibble %>% 
  {t.test(.$column1 ~ .$column2, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}
Risk_v <- tibble(
  mod  = character(),
  data.name = character(),
  p.value = numeric(),
  conf.int1 = numeric(),
  conf.int2 = numeric(),
  estimate1 = numeric(),
  estimate2 = numeric(),
  n = numeric()
)
#Alter the variables in the first line then hit run to make comparisons for different data-sets
```{r stats}
#Change test to all, non-dm or only dm then hit run for all comparisons
test <- Fracture_Dates_sum %>% filter(!is.na(Risk))
test <- test %>% mutate(Fracture = ifelse(Control == "TRUE", "Control", "Fracture"))
#Add column Fracture

#Make match controls and cases by age so that there is not a significant difference in age between the cohorts
#Think of a way to match the proportions of each 5 year increment, for example 10% of the fracture population is 50 years old, make this selection for the controls
test <- test %>% mutate(Decade = floor(ageatindex/10)*10)
Prop <- test %>%
  count(Fracture, Decade) %>%
  filter(Fracture == "Fracture") %>%
  mutate(Percent = (n/sum(n))*100)
#Select randomly from the control population based on these proportions.

G_v_Risk <- test %>% 
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}
ggplot(data = test, aes(x=ageatindex, fill=Control)) + 
  geom_histogram() +
  labs(title = "Age at index date", x="Age", y="Count") +
  scale_fill_manual(name="Legend",values=c("salmon","lightblue"),labels=c("Fracture","Control")) +
geom_text(aes(label=paste0("t.test p-value  ",round(G_v_Risk$p.value)), y=250, x=50))

#New values will be added to this tibble as you ask statistical questions
Desc_tibble <- tibble(
  Descriptor = character(),
  Control = numeric(),
  Exp = numeric(),
  p.value = numeric(),
  CI = character(),
  interpritation = character()
)

#number (n)
nExp <- test %>% filter(Control == "FALSE") %>% nrow()
nCont <- test %>% filter(Control == "TRUE") %>% nrow()

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "n",
  Control = nCont,
  Exp = nExp)

#%men
test2 <- test %>% filter(GENDER == 'M')
chi <- chisq.test(table(test2$GENDER, test2$Control))
Exp <- chi$observed[1]/nrow(test %>% filter(Control == "FALSE"))*100
Control <- chi$observed[2]/nrow(test %>% filter(Control == "TRUE"))*100
p.value <- chi$p.value[1]

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Men",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#%Women
test2 <- test %>% filter(GENDER == 'F')
chi <- chisq.test(table(test2$GENDER, test2$Control))
Exp <- chi$observed[1]/nrow(test %>% filter(Control == "FALSE"))*100
Control <- chi$observed[2]/nrow(test %>% filter(Control == "TRUE"))*100
p.value <- chi$p.value[1]

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Female",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#%White
test2 <- test %>% filter(Og_RACE == 'W')
chi <- chisq.test(table(test2$Og_RACE, test2$Control))
Exp <- chi$observed[1]/nrow(test %>% filter(Control == "FALSE"))*100
Control <- chi$observed[2]/nrow(test %>% filter(Control == "TRUE"))*100
p.value <- chi$p.value[1]

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% White",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#%Black
test2 <- test %>% filter(Og_RACE == 'B')
chi <- chisq.test(table(test2$Og_RACE, test2$Control))
Exp <- chi$observed[1]/nrow(test %>% filter(Control == "FALSE"))*100
Control <- chi$observed[2]/nrow(test %>% filter(Control == "TRUE"))*100

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Black",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)
rm(chi)

#Mean age
G_v_Risk <- test %>% 
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean Age",
  Exp = round(G_v_Risk$estimate[1], digits = 2),
  Control = round(G_v_Risk$estimate[2], digits = 2),
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean age Male
G_v_Risk <- test %>% filter(GENDER == 'M') %>%
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean Age Male",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean age female
G_v_Risk <- test %>% filter(GENDER == 'F') %>%
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean Age Female",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Diabetics
chi <- chisq.test(table(test$DM, test$Control))
Exp <- chi$observed[2]/nrow(test %>% filter(Control == "FALSE"))*100
Control <- chi$observed[4]/nrow(test %>% filter(Control == "TRUE"))*100
p.value <- chi$p.value[1]

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Diabetics",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Mean FRAX
G_v_Risk <- test %>%
  {t.test(.$Risk ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Diabetics
G_v_Risk <- test %>% filter(DM == "TRUE") %>%
  {t.test(.$Risk ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetic Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX nonDiabetics
G_v_Risk <- test %>% filter(DM == "FALSE") %>%
  {t.test(.$Risk ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetic Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Controls
G_v_Risk <- test %>% filter(Control == "TRUE") %>%
  {t.test(.$Risk ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Controls Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Fracture
G_v_Risk <- test %>% filter(Control == "FALSE") %>%
  {t.test(.$Risk ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Fracture Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Male
G_v_Risk <- test %>% filter(GENDER == "M") %>%
  {t.test(.$Risk ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Men Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Controls Male
G_v_Risk <- test %>% filter(Control == "TRUE" & GENDER == "M") %>%
  {t.test(.$Risk ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Controls, Men Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Mean FRAX Fracture Male
G_v_Risk <- test %>% filter(Control == "FALSE" & GENDER == "M") %>%
  {t.test(.$Risk ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Fracture, Men Mean FRAX Risk",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#FRAX>20
test <- test %>% mutate(threshold = ifelse(Risk > 20, "TRUE", "FALSE"))
chi <- chisq.test(table(test$threshold, test$Control))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% FRAX Risk > 20%",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Diabetics FRAX>20
test2 <- test %>% filter(DM == "TRUE")
chi <- chisq.test(table(test2$threshold, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics with % FRAX Risk > 20%",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#nonDiabetic FRAX>20
test2 <- test %>% filter(DM == "FALSE")
chi <- chisq.test(table(test2$threshold, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics with % FRAX Risk > 20%",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#FRAX_lower>20
test <- test %>% mutate(threshold = ifelse(Range_lower > 20, "TRUE", "FALSE"))
chi <- chisq.test(table(test$threshold, test$Control))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% FRAX Risk with lowest weighting > 20%",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Anti-osteo meds
  #Import and bind osteo_meds
Osteo_Meds_ed <- read_csv("C:/Users/acriscit/Documents/Results/Osteo_Meds_ed.csv")
Osteo_Meds_ed <-  Osteo_Meds_ed %>% mutate(Control = "TRUE")

Osteo_Meds_fx <- read_csv("C:/Users/acriscit/Documents/Results/Osteo_Meds_fx.csv")
Osteo_Meds_fx <-  Osteo_Meds_fx %>% mutate(Control = "FALSE")

Osteo_Meds <- bind_rows(Osteo_Meds_fx, Osteo_Meds_ed)
rm(Osteo_Meds_ed, Osteo_Meds_fx)
Osteo_Meds <- Osteo_Meds[-1]

  #Make a column for osteo_med start date
test <- Osteo_Meds %>% select(ID, osteo_med_start_date) %>% right_join(test, by = 'ID', .keep_all = FALSE)
  #Make a column to see if they were taking the med prior to fracture
test <- test %>% mutate(med_before = difftime(DATE, osteo_med_start_date, units = c("days")))
test <- test %>% mutate(true_med = ifelse(med_before > 0, "TRUE", "FALSE"))
test$true_med <- ifelse(is.na(test$true_med), "FALSE", test$true_med)

chi <- chisq.test(table(test$true_med, test$Control))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Taking anti-osteo med prior to fracture or index date",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Anti-osteo meds DM vs not
test2 <- test %>% filter(Control == "TRUE")
chi <- chisq.test(table(test2$true_med, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% Controls (DM vs nonDM) Taking anti-osteo med prior to index date",
  Exp = Exp,
  Control = Control,
  p.value = p.value)

test2 <- test %>% filter(Control == "FALSE")
chi <- chisq.test(table(test2$true_med, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2]+chi$observed[1])*100
Control <- chi$observed[4]/(chi$observed[3]+chi$observed[4])*100
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "% FX pts (DM vs nonDM) Taking anti-osteo med prior to index date",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)
rm(test2)

#Mean time on anti-osteo med prior to fracture
test2 <- test %>% filter(true_med == "TRUE")
G_v_Risk <- test %>% 
  filter(true_med == "TRUE") %>%
  {t.test(.$med_before ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- as.double(G_v_Risk$conf.int[1])
CI2 <- as.double(G_v_Risk$conf.int[2])
CI <- toString(c(CI1, CI2))
Exp <- as.double(G_v_Risk$estimate[1])
Control <- as.double(G_v_Risk$estimate[2])

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean days on anti-osteo med prior to first fracture or index date",
  Exp = Exp,
  Control = Control,
  p.value = as.double(G_v_Risk$p.value),
  CI = CI)
rm(test2)

#Mean BMI
G_v_Risk <- test %>%
  {t.test(.$BMI ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean BMI",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Diabetics Mean BMI
G_v_Risk <- test %>% filter(DM == "TRUE") %>%
  {t.test(.$BMI ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics Mean BMI",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#nonDiabetics Mean BMI
G_v_Risk <- test %>% filter(DM == "FALSE") %>%
  {t.test(.$BMI ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics Mean BMI",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#%DXA prior to fracture
#Need line to import DXA data
#DXA <- DXA[-1]
#DXA <- rename(DXA, DXA_DATE = DATE)
#test <- DXA %>% group_by(ID) %>% arrange(DXA_DATE, .group_by = TRUE) %>% distinct(ID, .keep_all = TRUE) %>% select(ID, DXA_DATE) %>% right_join(test, by = 'ID')
#test <- test %>% mutate(DXA_before_fx = DATE - DXA_DATE)
#test <- test %>% mutate(true_DXA = ifelse(DXA_before_fx > 0, "TRUE", "FALSE"))

#chi <- chisq.test(table(test$true_DXA, test$Control))
#Exp <- chi$observed[2]/nrow(test %>% filter(Control == "FALSE"))*100
#Control <- chi$observed[4]/nrow(test %>% filter(Control == "TRUE"))*100
#p.value <- chi$p.value

#Desc_tibble <- Desc_tibble %>% add_row(
#  Descriptor = "% with DXA prior to fracture",
#  Exp = Exp,
#  Control = Control,
#  p.value = p.value)

#Risk of death 1 year after fx
test <- test %>% mutate(time_to_death = ageatdeath-ageatindex) %>%
 mutate(Death1 = ifelse(time_to_death <= 1, "TRUE", "FALSE")) %>%
 mutate(Death5 = ifelse(time_to_death <= 5, "TRUE", "FALSE"))
test$Death1 <- ifelse(is.na(test$Death1), "FALSE", test$Death1)
test$Death5 <- ifelse(is.na(test$Death5), "FALSE", test$Death5)

chi <- chisq.test(table(test$Death1, test$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Risk of death in 1 year",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Diabetics risk of within 1 year
test2 <- test %>% filter(DM == "TRUE")
chi <- chisq.test(table(test2$Death1, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics Risk of death in 1 year",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#nonDiabetics risk of within 1 year
test2 <- test %>% filter(DM == "FALSE")
chi <- chisq.test(table(test2$Death1, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics Risk of death in 1 year",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Risk of death 5 years after fx
chi <- chisq.test(table(test$Death5, test$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Risk of death in 5 years",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Diabetics risk of within 5 year
test2 <- test %>% filter(DM == "TRUE")
chi <- chisq.test(table(test2$Death5, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics Risk of death in 5 years",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#nonDiabetics risk of within 5 year
test2 <- test %>% filter(DM == "FALSE")
chi <- chisq.test(table(test2$Death5, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics Risk of death in 5 years",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Risk of death overall
test <- test %>% mutate(Death = ifelse(is.na(ageatdeath), "FALSE", "TRUE"))
chi <- chisq.test(table(test$Death, test$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Risk of death overall",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#HBA1C vs Fracture
G_v_Risk <- DM_Dates_fx %>%
  {t.test(.$HBA1C_max ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Max HBA1C",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Long term insulin vs fracture
chi <- chisq.test(table(DM_Dates_fx$long_term_insulin, DM_Dates_fx$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Long term inslin vs fracture",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Diabetics Age vs fracture
G_v_Risk <- DM_Dates_fx %>% filter(DM == "TRUE") %>%
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean age of diabetics",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#nonDiabetics Age vs fracture
G_v_Risk <- Fracture_Dates_fx %>% filter(DM == "FALSE") %>%
  {t.test(.$ageatindex ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Mean age of non-diabetics",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Controls Age vs DM
G_v_Risk <- test %>% filter(Control == "TRUE") %>%
  {t.test(.$ageatindex ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Controls, Age vs DM",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

#Fracture Age vs DM
G_v_Risk <- test %>% filter(Control == "FALSE") %>%
  {t.test(.$ageatindex ~ .$DM, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Fracture, Age vs DM",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI,
  interpritation = "Diabetics with fracture are significantly younger, not true of controls")

#Diabetics sex vs fracture
test2 <- test %>% filter(DM == "TRUE")
chi <- chisq.test(table(test2$GENDER, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics, % Male",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#nonDiabetics sex vs fracture
test2 <- test %>% filter(DM == "FALSE")
chi <- chisq.test(table(test2$GENDER, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics % Male",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Controls sex vs DM
test2 <- test %>% filter(Control == "TRUE")
chi <- chisq.test(table(test2$GENDER, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Controls, % Male vs DM",
  Exp = Exp,
  Control = Control,
  p.value = p.value)

#Fracture sex vs DM
test2 <- test %>% filter(Control == "FALSE")
chi <- chisq.test(table(test2$GENDER, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Fracture, % Male vs DM",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value,
  interpritation = "There are more diabetic men who fracture compared to non-diabetic men")

#Diabetics race vs fracture
test2 <- test %>% filter(DM == "TRUE" & Og_RACE == "W"|Og_RACE == "B")
chi <- chisq.test(table(test2$Og_RACE, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Diabetics % White",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Nondiabetics race vs fracture
test2 <- test %>% filter(DM == "FALSE" & Og_RACE == "W"|Og_RACE == "B")
chi <- chisq.test(table(test2$Og_RACE, test2$Control))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "nonDiabetics % White",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value)

#Controls RACE vs DM
test2 <- test %>% filter(Control == "TRUE" & Og_RACE == "W"|Og_RACE == "B")
chi <- chisq.test(table(test2$Og_RACE, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Controls, % White vs DM",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value,
  interpritation = "Among controls, there is a higher proportion of white people without diabetes"
)

#Fracture Race vs DM
test2 <- test %>% filter(Control == "FALSE" & Og_RACE == "W"|Og_RACE == "B")
chi <- chisq.test(table(test2$Og_RACE, test2$DM))
Exp <- chi$observed[2]/(chi$observed[2] + chi$observed[1])
Control <- chi$observed[4]/(chi$observed[3] + chi$observed[4])
p.value <- chi$p.value

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "Fracture, % White vs DM",
  Exp = Exp,
  Control = Control,
  CI = as.character(chi$statistic),
  p.value = p.value,
  interpritation = "Among fracture there is a hight proportion of white people with diabetes")

test <- Visit_Dates %>% select(ID, CCI) %>% right_join(test, by = 'ID')
G_v_Risk <- test %>%
  {t.test(.$CCI ~ .$Control, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}

CI1 <- round(as.double(G_v_Risk$conf.int[1]), digits = 2)
CI2 <- round(as.double(G_v_Risk$conf.int[2]), digits = 2)
CI <- toString(c(CI1, CI2))

Desc_tibble <- Desc_tibble %>% add_row(
  Descriptor = "mean CCI at index date",
  Exp = G_v_Risk$estimate[1],
  Control = G_v_Risk$estimate[2],
  p.value = G_v_Risk$p.value,
  CI = CI)

write.csv(Desc_tibble, "C:\\Users\\acriscit\\Documents\\Desc_tibble.csv")
```
Below are scripts for chisq_test and Turkey test which are not working. The chi test would allow a data-table to be created for multiple pairwise tests. The Turkey test is a way to make many comparisons after an anova
```{r trying}
stat.test <- test %>%
  anova_test(Risk ~ Fracture + DM) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
stat.test <- stat.test %>% add_xy_position(x = "Fracture")
#Turkey https://www.scribbr.com/statistics/anova-in-r/
aov <- aov(Risk ~ DM_Control, data = smooth)
turkey<-TukeyHSD(aov)
#Plot it
plot(turkey, las = 1)

#Doesnt work
Prop <- Prop %>% unite("Match", c(GENDER, Fracture, DM), remove = TRUE) %>% select(!(Percent))
stat.test <- Prop %>%
  chisq_test() %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
stat.test <- stat.test %>% add_xy_position(x = "Fracture")
```
Histogram
```{r histogram}
ggplot(data = test, aes(x=Risk, fill=Control)) + 
  geom_histogram() +
  labs(title = "FRAX Risk Score", x="FRAX score", y="Count") +
  scale_fill_manual(name="Legend",values=c("salmon","lightblue"),labels=c("Fracture","Control"))
#Age vs count
ggplot(data = test, aes(x=ageatindex, fill=Control)) + 
  geom_histogram() +
  labs(title = "Age at index date", x="Age", y="Count") +
  scale_fill_manual(name="Legend",values=c("salmon","lightblue"),labels=c("Fracture","Control"))


#Smooth is characterized lower down
ggplot(data = smooth, aes(x=Risk, fill=c(DM_Control))) + 
  geom_histogram() +
  labs(title = "FRAX vs DM and Control", x="Age", y="Count") +
  scale_fill_manual(name="Legend",values=c("salmon","lightblue"),labels=c("Fracture","Control"))
```
Kolmogrov-Smirnov test for normality on above data
```{r normal}

ks.test(test$Risk, 'pnorm', mean=mean(test$Risk), sd=sd(test$Risk))

shapiro.test(test$Risk)

#test for skew of the data
install.packages("tseries")
library(tseries)
jarque.bera.test(test$Risk)
```
Chronology
```{r chron}
  
rm(test)
#Create a histogram that outlines the chronology of fractures
test$DATE <- test$DATE %>% as.Date()
library(scales)

date_mean <- test %>% filter(Control == "FALSE")
date_mean <- date_mean$DATE %>% mean()

ggplot(test %>% filter(Control == "FALSE"), aes(x=DATE)) + 
  geom_histogram(binwidth=30, colour="lightblue") +
  scale_x_date(labels = date_format("%Y")) +
  labs(title = "Fracture Chonology", x="Time", y="Count") +
  
  geom_vline(xintercept=date_mean, size=1, color="darkblue")+
  geom_text(aes(x=date_mean - 3000,label=paste0("Mean:  ",date_mean), y=20))

```
Box plots
```{r plot}
plot_tibble <- tibble(
  Descriptor = character(),
  Control = numeric(),
  Exp = numeric(),
  p.value = numeric(),
  CI = character()
)

#PLOT1: Control vs Frax score box plot (Chnage Control to fracture vs no fracture)

library(rstatix)
library(ggpubr)
stat.test <- test %>%
  select(Fracture, Risk) %>%
  t_test(Risk ~ Fracture) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
stat.test <- stat.test %>% add_xy_position(x = "Fracture")

ggplot(data = test, aes(Control, Risk)) +
geom_violin(width=1, col="black",  fill="dodgerblue", alpha = 0.1) +
geom_boxplot(col="blue", alpha = .5) +
  labs(title = "Fracture vs FRAX score", x="", y="FRAX") +
  scale_x_discrete(labels=c("Fracture", "Control")) +
  stat_pvalue_manual(stat.test, label = "p.adj.signif")

#Make 4 plots based on Control and sex
smooth <- test %>% unite('SEX_Control', c(Control, GENDER), remove = FALSE) %>%
  unite('DM_Control', c(Fracture,DM), remove = FALSE) %>%
  unite('Race_Control', c(Fracture,Og_RACE), remove =  FALSE) %>%
  unite('DM_SEX', c(DM, GENDER), remove =  FALSE) %>%
  filter (!is.na(Risk)) %>%
  mutate(Decade = floor(ageatindex/10)*10)

 # unite('Match', c(GENDER,Og_RACE,Decade), sep = "", remove = FALSE) %>% arrange(Match)

#Plot 2: Diabetic Status vs FRAX score
ggplot(data = smooth, aes(DM_Control, Risk, fill = Fracture)) +
geom_boxplot() +
  labs(title = "Diabetic Status vs FRAX score", x="", y="FRAX") +
  scale_x_discrete(labels=c("Healthy Control","Diabetic Control", "Healthy Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))

#PLOT3: Age stratified by DM and Fracture
ggplot(data = smooth, aes(DM_Control,ageatindex, fill = Fracture)) +
  geom_violin(width=1,
                 col="dodgerblue", 
                 fill="dodgerblue", 
                 alpha = .5) +
geom_boxplot(alpha = .5) +
  labs(title = "Age at Index Date", x="", y="Age") +
  scale_x_discrete(labels=c("Healthy Control", "Diabetic Control", "Healthy Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))

#PLOT6: Sex and DM as a %
Prop <- test %>% count(GENDER, Fracture, DM)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = Fracture, y = Percent, fill = DM)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Sex and Diabetes", x="", y="Percent (%)", fill= "DM") +
  scale_x_discrete(guide = guide_axis(n.dodge=4)) +
  facet_wrap(~GENDER)

#PLOT7: Race and DM as a %
Prop <- test %>% filter(Og_RACE == "W" | Og_RACE == "B") %>% count(Og_RACE, Fracture, DM)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = Fracture, y = Percent, fill = DM)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Race and Diabetes", x="", y="Percent (%)", fill= "DM") +
  scale_x_discrete(guide = guide_axis(n.dodge=2))+
  facet_wrap(~Og_RACE)

#PLOT8: DM and Fracture
Prop <- test %>% count(Fracture, DM)
Prop <- Prop %>% mutate(Percent = (n/sum(Prop$n))*100)

ggplot(Prop, aes(x = Fracture, y = Percent, fill = DM)) +  
  geom_bar(position="dodge", stat="identity") +
  labs(title = "Distribution of Diabetes", x="", y="Percent (%)", fill= "DM")

#PLOT9: FRAX vs DM, Sex and Fracture
plot_tibble <- plot_tibble %>% add_row(
  Descriptor = "PLOT9")

ggplot(data = smooth, aes(DM_Control,Risk, fill = GENDER)) +
geom_boxplot() +
  labs(title = "Sex vs FRAX score", x="", y="FRAX") +
  scale_x_discrete(labels=c("Healthy Control","Diabetic Control","Health Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))

#PLOT10: FRAX vs DM, Race and Fracture
plot_tibble <- plot_tibble %>% add_row(
  Descriptor = "PLOT10")

smooth2 <- smooth %>% filter(Og_RACE == "W" | Og_RACE == "B")

ggplot(data = smooth2, aes(DM_Control,Risk, fill = Og_RACE)) +
geom_boxplot() +
  labs(title = "Race vs FRAX score", x="", y="FRAX", fill= "Race") +
  scale_x_discrete(labels=c("Healthy Control","Diabetic Control","Health Fracture","Diabetic Fracture"), guide = guide_axis(n.dodge=2))
rm(smooth2)

#PLOT11: Age vs FRAX by diabets and fracture
#Figure out how to create a line of boxplots to represent this data
ggplot(data = smooth, aes(ageatindex, Risk, colour = DM_Control)) +
geom_smooth(method = 'gam') +
xlab("Age (Years)") +
ylab("FRAX") +
ggtitle("Age vs FRAX")

#Age (5 years), sex and race matched control

x <- smooth %>% filter(Five <= 80) %>%
  filter(Og_RACE == "B" | Og_RACE == "W") %>%
  #filter(Fracture == "Fracture") %>%
  select(Risk, Match, Fracture, DM)

library(rstatix)
library(ggpubr)
stat.test <- x %>%
  t_test(Risk ~ Fracture) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
view(stat.test)

myplot <- ggboxplot(
  x, x = "Fracture", y = "Risk",
  fill = "Fracture", palette = "npg", legend = "none",
  ggtheme = theme_pubr(border = TRUE)
  ) +
  scale_x_discrete(guide = guide_axis(n.dodge=2)) +
  theme(axis.text.y = element_blank(),
        axis.text.x = element_text(size=8)) +
  labs(title = "DM, Sex and race matched", x="", y="FRAX") +
  facet_wrap(~Match)
stat.test <- stat.test %>% add_xy_position(x = "Fracture")
myplot + stat_pvalue_manual(stat.test, label = "p.adj.signif")

rm(stat.test, myplot, x)
```
ROC
```{r ROC}
library(pROC)
#ROC Control vs FRAX (fracture for Control)
roc <- test %>%
  roc("Fracture", "Risk",
      level=c("Control", "Fracture"),
      direction='<', #means that observations are positive when greater than the threshold
      thresholds=100, auc = TRUE, ci = TRUE)

#sens and spec for threshold of 20, "all" for all thresholds, "best" for best thresholds
coords(roc, "best", ret=c("threshold", "specificity", "sensitivity", "accuracy", "precision", "recall"), transpose = FALSE)

#sensitivity of 50%
coords(roc=roc, x=50, input="sensitivity", transpose = FALSE)

#Plot the thresholds, can also put "best" or "all"
plot(roc, print.thres=20)
#Another way to plot ROC
ggroc(roc) +
  ggtitle ("FRAX score ROC, Control vs Fracture")
#plot sensitivity and specificity vs threshold
plot(specificity + sensitivity ~ threshold, 
      coords(roc, "all", transpose = FALSE), 
      type = "l", log="x", 
      subset = is.finite(threshold))
#percision recall plot
plot(precision ~ recall, #Percision is TRUE pos/ALL pos. Recall is the same as sensitivity
      coords(roc, "all", ret = c("recall", "precision"), transpose = FALSE),
      type="l")
#tpr/fpr
plot(tpr ~ fpr, 
     coords(roc, "all", ret = c("tpr", "fpr"), transpose = FALSE),
     type="l")

#ROC, DM vs noDM
rocNDM <- test %>%
  filter(DM == "FALSE") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
rocDM <- test %>% filter(DM == "TRUE") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
roc.test(rocNDM, rocDM, reuse.auc=FALSE,#ensures that auc from the prior operations are not used
         method="delong") #D=(AUC1-AUC2)/s
#method="bootstrap", boot.stratified=FALSE
#Plot
plot(rocNDM, print.thres=20)
plot(rocDM, print.thres=20)

#ROC, Sex
#Automatically sets Controls < cases (meaning it makes the group with lower mean FRAX the controls. In this case, these are actually our controls)
rocM <- test %>%
  filter(GENDER == "M") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
rocF <- test %>% filter(GENDER == "F") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
roc.test(rocM, rocF, reuse.auc=FALSE,#ensures that auc from the prior operations are not used
         method="delong") #D=(AUC1-AUC2)/s
#plot
plot(rocM, print.thres=20, print.auc=TRUE)
plot(rocF, print.thres=20)

#ROC, Race
rocB <- test %>%
  filter(Og_RACE == "B") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
rocW <- test %>% filter(Og_RACE == "W") %>%
  roc(Fracture, Risk, levels=c("Control", "Fracture"))
roc.test(rocB, rocW, reuse.auc=FALSE,#ensures that auc from the prior operations are not used
         method="delong") #D=(AUC1-AUC2)/s
#plot
plot(rocB, print.thres=20, print.auc=TRUE)
plot(rocW, print.thres=20)

#record thresholds
best_f <- function(x){coords(x, "best", ret=c("threshold", "specificity", "sensitivity"), transpose = FALSE)}
#threshold of 20
twenty_f <- function(x){coords(roc=x, x=20, input="threshold", transpose = FALSE)}
ROC_tibble <- tibble(
  Desc = character(),
  auc = numeric(),
  Best_sens = numeric(),
  Best_spec = numeric(),
  Best_thres = numeric(),
  Twenty_sens = numeric(),
  Twenty_spec = numeric(),
)

best <- best_f(rocDM)
twenty <- twenty_f(rocDM)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "DM",
  auc = as.double(rocDM$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)
best <- best_f(rocNDM)
twenty <- twenty_f(rocNDM)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "nDM",
  auc = as.double(rocNDM$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)
best <- best_f(rocM)
twenty <- twenty_f(rocM)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "Male",
  auc = as.double(rocM$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)
best <- best_f(rocF)
twenty <- twenty_f(rocF)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "Female",
  auc = as.double(rocM$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)

best <- best_f(rocB)
twenty <- twenty_f(rocB)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "Black",
  auc = as.double(rocB$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)

best <- best_f(rocW)
twenty <- twenty_f(rocW)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "White",
  auc = as.double(rocW$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)


#ROC range_upper (heaviest weighting)
roc_upper <- test %>% filter(GENDER == "F") %>%
  roc(Control, Range_upper, levels=c("TRUE", "FALSE")) 
coords(roc, "best", ret=c("threshold", "sensitivity", "specificity"))

roc6.5 <- DM_Dates_fx %>% filter(HBA1C_max > 6.5) %>%
  roc(Control, Risk, levels=c("TRUE", "FALSE")) 
roc.test(roc6.5,roc, reuse.auc=FALSE,#ensures that auc from the prior operations are not used
         method="delong") #D=(AUC1-AUC2)/s
best <- best_f(roc6.5)
twenty <- twenty_f(roc6.5)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "HBA1C 6.5",
  auc = as.double(roc6.5$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)

roc8 <- DM_Dates_fx %>% filter(HBA1C_max > 8) %>%
  roc(Control, Risk, levels=c("TRUE", "FALSE"))
coords(roc8, "best", ret=c("threshold", "sensitivity", "specificity"))
roc.test(roc8,roc, reuse.auc=FALSE,#ensures that auc from the prior operations are not used
         method="delong", #D=(AUC1-AUC2)/s
         #alternative="less" #can add alternative to specify the direction of the comparison. for example "less" means the alternative hypothesis is that AUC1 is less than AUC2 or that FRAX is a poorer predictor of Fracture in group 1 than 2
         )
best <- best_f(roc8)
twenty <- twenty_f(roc8)
ROC_tibble <- ROC_tibble %>% add_row(
  Desc = "HBA1C 8",
  auc = as.double(roc8$auc),
  Best_sens = best$sensitivity,
  Best_spec = best$specificity,
  Best_thres = best$threshold,
  Twenty_sens = twenty$sensitivity,
  Twenty_spec = twenty$specificity,
)

view(ROC_tibble)


#ROC Control vs FRAX -5 years (substitute fracture for Control)
#You can do this for each year leading up to fracture and determine the predictive value of FRAX for each year

```
survival curve
```{r survival}

```
Linear regression
```{r regression}
#Implement multiple regression
#Regress FRAX vs comorbidities
#time to death vs FRAX Risk score
test2 <-  test %>% filter(time_to_death > 0)
lm_FRAX <- lm(Risk ~ time_to_death, data = test2)
summary <- summary(lm_FRAX)

ggplot(lm_FRAX, aes(x=time_to_death, y=Risk)) +
  geom_point(size = 0.25)+
  geom_smooth(method="lm", color = "#9D7C2F") +
  xlab("Time (years)") +
  ylab("FRAX score") +
  theme_bw() +
  ggtitle("Time to Death vs FRAX score") +
  geom_text(aes(label=paste0("R^2:  ",round(summary$r.squared, digits = 3)), y=80, x=10))
rm(test2)

#FRAX Score vs time to death
lm_FRAX <- lm(time_to_death ~ Risk , data = test %>% filter(time_to_death >= 0 & Control == "FALSE" & DM == "FALSE"))
summary <- summary(lm_FRAX)

ggplot(lm_FRAX, aes(x=time_to_death, y=Risk)) +
  geom_point(size = 0.25)+
  geom_smooth(method="lm", color = "#9D7C2F") +
  xlab("Time to death (years)") +
  ylab("FRAX score") +
  theme_bw() +
  ggtitle("Patients with Fracture, not DM, Time to death vs FRAX score") +
  geom_text(aes(label=paste0("R^2:  ",round(summary$r.squared, digits = 3)), y=80, x=20))

#In the control group, people fractured after being seen in the ED. The below graph depicts the correlation between "time to fracture" and FRAX score
#select patients who fractured after ED visit
lm_FRAX <- lm(fx_time ~ Risk , data = Controls_with_MOF %>% filter(fx_time >= 1))
summary <- summary(lm_FRAX)
n <- nrow(lm_FRAX$model)

ggplot(lm_FRAX, aes(x=fx_time, y=Risk)) +
  geom_point(size = 0.25)+
  geom_smooth(method="lm", color = "#9D7C2F") +
  xlab("Time to fracture (days)") +
  ylab("FRAX score") +
  theme_bw() +
  ggtitle("Time from index date to fracture") +
  geom_text(aes(label=paste0("R^2:  ",round(summary$r.squared, digits = 3)), y=60, x=2000))

#Same as above but further selected for patients with pathologic fracture
lm_FRAX <- lm(fx_time ~ Risk , data = test %>% filter(fx_time >= 1) %>% filter(grepl('[Pp]athologic fracture', dx_DESC)))
summary <- summary(lm_FRAX)
n <- nrow(lm_FRAX$model)

fx_plot <- ggplot(lm_FRAX, aes(x=fx_time, y=Risk)) +
  geom_point(size = 0.25)+
  geom_smooth(method="lm", color = "#9D7C2F") +
  xlab("Time to fracture (days)") +
  ylab("FRAX score") +
  theme_bw() +
  ggtitle("Time from index date to pathologic fracture") +
  geom_text(aes(label=paste0("R^2:  ",round(summary$r.squared, digits = 3)), y=60, x=2000))

#FRAX vs Fracture among control patients
test <- test %>% mutate(fx = ifelse(is.na(fx_DESC), "FALSE", "TRUE"))
test2 <- test %>% filter(Control == TRUE)
ttest <- test2 %>% 
  {t.test(.$Risk ~ .$fx, alternative = c("two.sided"),
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)}
chi <- chisq.test(table(test2$threshold, test2$fx))

fx_plot + geom_text(aes(label=paste0("t.test p-value  ",round(ttest$p.value)), y=30, x=2000))

#Plot a histogram of controls who did and did not fracture
ggplot(data = test2, aes(x=Risk, fill=fx)) + 
  geom_histogram() +
  labs(title = "FRAX Risk Score", x="FRAX score", y="Count") +
  scale_fill_manual(name="Legend",values=c("salmon","lightblue"),labels=c("No Fracture","Fracture"))

#Finally, use your cohort of non-fractured diabetics to assess the predictive value of FRAX with
  #1) A survival curve (where fracture is supplemented for survicval)
  #2) Linear regression of FRAX score vs time to fracture from index date

######################################################################
```
Osteo_meds
```{r meds}
#Osteo_meds should have been uploaded prior

ggplot(data = test %>% filter(med_before >= 0), aes(Risk, Control)) +
  geom_violin(col="dodgerblue", 
                 fill="dodgerblue", 
                 alpha = .5) +
  labs(title = "FRAX Risk Score", x="FRAX score", y="Anti-osteoporotic Medication") +
  scale_x_discrete(labels=c("No DM fracture","No DM control","DM fracture","DM control"))


  stat_boxplot(width=.2, col="black", fun.data = mean_se, geom = "errorbar", size=.5) +
  geom_boxplot(width=.3,
                 col="black", 
                 fill="dodgerblue", 
                 alpha = 1)



#Antiosteoporotic meds for pts with risk > 20
ggplot() +
  geom_violin(data = Fracture_Dates_fx20, aes(Risk,oste_med),
                 col="blue", 
                 fill="blue", 
                 alpha = .4) +
  labs(title = "Patients with FRAX Risk > 20%", x="FRAX score", y="Anti-osteoporotic Medication") +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))

#Box plot Control, sex and FRAX score
smooth <- Fracture_Dates_fx %>% unite('SEX_Control', c(GENDER,Control), remove = TRUE) %>% select(ID,SEX_Control,AGE,Risk) %>% filter (!is.na(Risk))

smooth_plot <- ggplot(data = smooth, aes(SEX_Control,AGE)) +
geom_boxplot() +
  labs(title = "Sex, Control and Age", x="Sex and Diabetes", y="Age") +
  scale_x_discrete(labels=c("F Control","F","M Control","M"))
```
Plotting a smooth curve
```{r smooth}
smooth <- Fracture_Dates_fx %>% unite('SEX_Control', c(GENDER,Control), remove = TRUE) %>% select(ID,SEX_Control,AGE,Risk) %>% pivot_longer(cols = c(SEX_Control,AGE,Risk), names_to = "Variable", values_to = "Value", values_drop_na = TRUE)

ggplot(Fracture_Dates_fx, aes(x=AGE, y=Risk)) +
  #geom_point() +
  geom_smooth(se=FALSE, col='red', size=2) +
  geom_hline(xintercept, linetype, color, size)